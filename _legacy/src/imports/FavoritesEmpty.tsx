import svgPaths from "./svg-wuo8xuetxh";
import imgImageWithFallback from "figma:asset/7ee6f82a1d36464ccf2f438cabbc2624b2346089.png";
import imgImageWithFallback1 from "figma:asset/0840806fb802d9ede186a567f73c151018ca084d.png";
import imgImageWithFallback2 from "figma:asset/e3f714599dbd4cb999076f94de3dc3a20a12a2c1.png";
import imgImageWithFallback3 from "figma:asset/64dca72a9f78c6a38f2f757f161019ebc88de3fb.png";
import imgImageWithFallback4 from "figma:asset/4fa8b39e9c2ac96748ac43f791167a77131c590e.png";
import imgImageWithFallback5 from "figma:asset/22e8cdddd9ab3900abde04f193e6b75a535503ff.png";
import imgImageWithFallback6 from "figma:asset/f088d7ec9f16661bc461a762417ac9e90242645d.png";
import imgImageWithFallback7 from "figma:asset/432e520b35ca5db7bb27272f597e5876cda9dd9a.png";
import imgImageWithFallback8 from "figma:asset/4bccc138d7d406e34e8558c132be301bebc7f3d7.png";
import imgImageWithFallback9 from "figma:asset/46ea705ffd78f4019918bb2fd2b66e6caa50008e.png";
import imgImageWithFallback10 from "figma:asset/91bb79150f84dc69e298818199169189c32fd5d7.png";
import imgImageWithFallback11 from "figma:asset/c90671806a221dfe4f2cfa2c496b9afe21957596.png";
import imgImageWithFallback12 from "figma:asset/60aeb6e8fae869d59657e4792208dcffbd4ed8de.png";
import imgImageWithFallback13 from "figma:asset/eb34852b4e297ee7b9fefd08cceddc7281c77866.png";
import imgImageWithFallback14 from "figma:asset/049a6e5cda93fc93492b0452f06f9ce2d1c062e4.png";
import imgImageWithFallback15 from "figma:asset/bfb0a76ed33c7802b805fae5c2717c5f337ffb87.png";
import imgImageWithFallback16 from "figma:asset/9b9e29faabe6d02fbddb40d7181f00f5169c1042.png";
import imgImageWithFallback17 from "figma:asset/024d1179f5b1864e85e8d66a4862ca6b0409319c.png";
import imgImageWithFallback18 from "figma:asset/d3e8201b26e4bdb300ee825201cf21d582bfda16.png";
import imgImageWithFallback19 from "figma:asset/f09776468c431cd80517f08553b3afb7b61d6f44.png";
import imgImageWithFallback20 from "figma:asset/24f726cdcbea5949b9b6f1df05e29d7f53d7b8bc.png";
import imgImageWithFallback21 from "figma:asset/3468de44fa3a04d3198a56a23dfc70c4d84eb9c8.png";

function Heading1() {
  return (
    <div className="absolute h-[30px] left-0 top-0 w-[1408px]" data-name="Heading 2">
      <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[30px] left-[704.31px] text-[20px] text-center text-neutral-950 text-nowrap top-0 tracking-[-0.48px] translate-x-[-50%] whitespace-pre">Shop by Collection</p>
    </div>
  );
}

function Paragraph() {
  return (
    <div className="absolute h-[24px] left-[368px] top-[46px] w-[672px]" data-name="Paragraph">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[336.41px] text-[#717182] text-[16px] text-center text-nowrap top-[-1px] tracking-[-0.48px] translate-x-[-50%] whitespace-pre">Explore our curated collections of premium NEAR Protocol merchandise</p>
    </div>
  );
}

function Container() {
  return (
    <div className="h-[70px] relative shrink-0 w-full" data-name="Container">
      <Heading1 />
      <Paragraph />
    </div>
  );
}

function ImageWithFallback() {
  return (
    <div className="absolute h-[517.5px] left-0 top-0 w-[690px]" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback} />
    </div>
  );
}

function Container1() {
  return <div className="absolute bg-gradient-to-t from-[rgba(0,0,0,0.6)] h-[517.5px] left-0 to-[rgba(0,0,0,0)] top-0 w-[690px]" data-name="Container" />;
}

function Heading2() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-nowrap text-white top-[-1px] tracking-[-0.48px] whitespace-pre">Men</p>
    </div>
  );
}

function Paragraph1() {
  return (
    <div className="h-[20px] relative shrink-0 w-full" data-name="Paragraph">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[20px] left-0 text-[14px] text-[rgba(255,255,255,0.8)] top-0 tracking-[-0.48px] w-[80px]">4 Products</p>
    </div>
  );
}

function Container2() {
  return (
    <div className="h-[48px] relative shrink-0 w-[79.203px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[48px] items-start relative w-[79.203px]">
        <Heading2 />
        <Paragraph1 />
      </div>
    </div>
  );
}

function Icon() {
  return (
    <div className="relative shrink-0 size-[20px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 20 20">
        <g id="Icon">
          <path d="M4.16667 10H15.8333" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.66667" />
          <path d={svgPaths.p1ae0b780} id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.66667" />
        </g>
      </svg>
    </div>
  );
}

function Container3() {
  return (
    <div className="absolute content-stretch flex h-[48px] items-center justify-between left-[24px] top-[445.5px] w-[642px]" data-name="Container">
      <Container2 />
      <Icon />
    </div>
  );
}

function CollectionCard() {
  return (
    <div className="absolute bg-[#ececf0] h-[517.5px] left-px overflow-clip top-px w-[690px]" data-name="CollectionCard">
      <ImageWithFallback />
      <Container1 />
      <Container3 />
    </div>
  );
}

function Card() {
  return (
    <div className="[grid-area:1_/_1] bg-white relative shrink-0" data-name="Card">
      <div className="overflow-clip relative rounded-[inherit] size-full">
        <CollectionCard />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function ImageWithFallback1() {
  return (
    <div className="absolute h-[517.5px] left-0 top-0 w-[690px]" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback1} />
    </div>
  );
}

function Container4() {
  return <div className="absolute bg-gradient-to-t from-[rgba(0,0,0,0.6)] h-[517.5px] left-0 to-[rgba(0,0,0,0)] top-0 w-[690px]" data-name="Container" />;
}

function Heading4() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-nowrap text-white top-[-1px] tracking-[-0.48px] whitespace-pre">Women</p>
    </div>
  );
}

function Paragraph2() {
  return (
    <div className="h-[20px] relative shrink-0 w-full" data-name="Paragraph">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[20px] left-0 text-[14px] text-[rgba(255,255,255,0.8)] top-0 tracking-[-0.48px] w-[80px]">4 Products</p>
    </div>
  );
}

function Container5() {
  return (
    <div className="h-[48px] relative shrink-0 w-[79.203px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[48px] items-start relative w-[79.203px]">
        <Heading4 />
        <Paragraph2 />
      </div>
    </div>
  );
}

function Icon1() {
  return (
    <div className="relative shrink-0 size-[20px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 20 20">
        <g id="Icon">
          <path d="M4.16667 10H15.8333" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.66667" />
          <path d={svgPaths.p1ae0b780} id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.66667" />
        </g>
      </svg>
    </div>
  );
}

function Container6() {
  return (
    <div className="absolute content-stretch flex h-[48px] items-center justify-between left-[24px] top-[445.5px] w-[642px]" data-name="Container">
      <Container5 />
      <Icon1 />
    </div>
  );
}

function CollectionCard1() {
  return (
    <div className="absolute bg-[#ececf0] h-[517.5px] left-px overflow-clip top-px w-[690px]" data-name="CollectionCard">
      <ImageWithFallback1 />
      <Container4 />
      <Container6 />
    </div>
  );
}

function Card1() {
  return (
    <div className="[grid-area:1_/_2] bg-white relative shrink-0" data-name="Card">
      <div className="overflow-clip relative rounded-[inherit] size-full">
        <CollectionCard1 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function ImageWithFallback2() {
  return (
    <div className="absolute h-[517.5px] left-0 top-0 w-[690px]" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback2} />
    </div>
  );
}

function Container7() {
  return <div className="absolute bg-gradient-to-t from-[rgba(0,0,0,0.6)] h-[517.5px] left-0 to-[rgba(0,0,0,0)] top-0 w-[690px]" data-name="Container" />;
}

function Heading5() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-nowrap text-white top-[-1px] tracking-[-0.48px] whitespace-pre">Exclusives</p>
    </div>
  );
}

function Paragraph3() {
  return (
    <div className="h-[20px] relative shrink-0 w-full" data-name="Paragraph">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[20px] left-0 text-[14px] text-[rgba(255,255,255,0.8)] top-0 tracking-[-0.48px] w-[80px]">3 Products</p>
    </div>
  );
}

function Container8() {
  return (
    <div className="h-[48px] relative shrink-0 w-[91.203px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[48px] items-start relative w-[91.203px]">
        <Heading5 />
        <Paragraph3 />
      </div>
    </div>
  );
}

function Icon2() {
  return (
    <div className="relative shrink-0 size-[20px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 20 20">
        <g id="Icon">
          <path d="M4.16667 10H15.8333" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.66667" />
          <path d={svgPaths.p1ae0b780} id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.66667" />
        </g>
      </svg>
    </div>
  );
}

function Container9() {
  return (
    <div className="absolute content-stretch flex h-[48px] items-center justify-between left-[24px] top-[445.5px] w-[642px]" data-name="Container">
      <Container8 />
      <Icon2 />
    </div>
  );
}

function CollectionCard2() {
  return (
    <div className="absolute bg-[#ececf0] h-[517.5px] left-px overflow-clip top-px w-[690px]" data-name="CollectionCard">
      <ImageWithFallback2 />
      <Container7 />
      <Container9 />
    </div>
  );
}

function Card2() {
  return (
    <div className="[grid-area:2_/_1] bg-white relative shrink-0" data-name="Card">
      <div className="overflow-clip relative rounded-[inherit] size-full">
        <CollectionCard2 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function ImageWithFallback3() {
  return (
    <div className="absolute h-[517.5px] left-0 top-0 w-[690px]" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback3} />
    </div>
  );
}

function Container10() {
  return <div className="absolute bg-gradient-to-t from-[rgba(0,0,0,0.6)] h-[517.5px] left-0 to-[rgba(0,0,0,0)] top-0 w-[690px]" data-name="Container" />;
}

function Heading6() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-nowrap text-white top-[-1px] tracking-[-0.48px] whitespace-pre">Accessories</p>
    </div>
  );
}

function Paragraph4() {
  return (
    <div className="h-[20px] relative shrink-0 w-full" data-name="Paragraph">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[20px] left-0 text-[14px] text-[rgba(255,255,255,0.8)] top-0 tracking-[-0.48px] w-[80px]">7 Products</p>
    </div>
  );
}

function Container11() {
  return (
    <div className="h-[48px] relative shrink-0 w-[100.328px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[48px] items-start relative w-[100.328px]">
        <Heading6 />
        <Paragraph4 />
      </div>
    </div>
  );
}

function Icon3() {
  return (
    <div className="relative shrink-0 size-[20px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 20 20">
        <g id="Icon">
          <path d="M4.16667 10H15.8333" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.66667" />
          <path d={svgPaths.p1ae0b780} id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.66667" />
        </g>
      </svg>
    </div>
  );
}

function Container12() {
  return (
    <div className="absolute content-stretch flex h-[48px] items-center justify-between left-[24px] top-[445.5px] w-[642px]" data-name="Container">
      <Container11 />
      <Icon3 />
    </div>
  );
}

function CollectionCard3() {
  return (
    <div className="absolute bg-[#ececf0] h-[517.5px] left-px overflow-clip top-px w-[690px]" data-name="CollectionCard">
      <ImageWithFallback3 />
      <Container10 />
      <Container12 />
    </div>
  );
}

function Card3() {
  return (
    <div className="[grid-area:2_/_2] bg-white relative shrink-0" data-name="Card">
      <div className="overflow-clip relative rounded-[inherit] size-full">
        <CollectionCard3 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Container13() {
  return (
    <div className="gap-[24px] grid grid-cols-[repeat(2,_minmax(0px,_1fr))] grid-rows-[repeat(2,_minmax(0px,_1fr))] h-[1063px] relative shrink-0 w-full" data-name="Container">
      <Card />
      <Card1 />
      <Card2 />
      <Card3 />
    </div>
  );
}

function Button() {
  return (
    <div className="absolute bg-neutral-950 box-border content-stretch flex gap-[8px] h-[36px] items-center justify-center left-[477.27px] px-[16px] py-[8px] top-0 w-[55.766px]" data-name="Button">
      <p className="font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] relative shrink-0 text-[14px] text-nowrap text-white tracking-[-0.48px] whitespace-pre">All</p>
    </div>
  );
}

function Button1() {
  return (
    <div className="absolute bg-white box-border content-stretch flex gap-[8px] h-[36px] items-center justify-center left-[541.03px] px-[17px] py-[9px] top-0 w-[57.766px]" data-name="Button">
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
      <p className="font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] relative shrink-0 text-[14px] text-neutral-950 text-nowrap tracking-[-0.48px] whitespace-pre">Men</p>
    </div>
  );
}

function Button2() {
  return (
    <div className="absolute bg-white box-border content-stretch flex gap-[8px] h-[36px] items-center justify-center left-[606.8px] px-[17px] py-[9px] top-0 w-[73.609px]" data-name="Button">
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
      <p className="font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] relative shrink-0 text-[14px] text-neutral-950 text-nowrap tracking-[-0.48px] whitespace-pre">Women</p>
    </div>
  );
}

function Button3() {
  return (
    <div className="absolute bg-white box-border content-stretch flex gap-[8px] h-[36px] items-center justify-center left-[688.41px] px-[17px] py-[9px] top-0 w-[113.203px]" data-name="Button">
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
      <p className="font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] relative shrink-0 text-[14px] text-neutral-950 text-nowrap tracking-[-0.48px] whitespace-pre">Exclusives</p>
    </div>
  );
}

function Button4() {
  return (
    <div className="absolute bg-white box-border content-stretch flex gap-[8px] h-[36px] items-center justify-center left-[809.61px] px-[17px] py-[9px] top-0 w-[121.125px]" data-name="Button">
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
      <p className="font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] relative shrink-0 text-[14px] text-neutral-950 text-nowrap tracking-[-0.48px] whitespace-pre">Accessories</p>
    </div>
  );
}

function Container14() {
  return (
    <div className="h-[36px] relative shrink-0 w-full" data-name="Container">
      <Button />
      <Button1 />
      <Button2 />
      <Button3 />
      <Button4 />
    </div>
  );
}

function Container15() {
  return (
    <div className="h-[1265px] relative shrink-0 w-full" data-name="Container">
      <div className="size-full">
        <div className="box-border content-stretch flex flex-col gap-[48px] h-[1265px] items-start px-[64px] py-0 relative w-full">
          <Container />
          <Container13 />
          <Container14 />
        </div>
      </div>
    </div>
  );
}

function Section() {
  return (
    <div className="absolute box-border content-stretch flex flex-col h-[1426px] items-start left-0 pb-px pt-[80px] px-[11px] top-[669px] w-[1558px]" data-name="Section">
      <div aria-hidden="true" className="absolute border-[0px_0px_1px] border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
      <Container15 />
    </div>
  );
}

function Paragraph5() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Men</p>
    </div>
  );
}

function Heading7() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">NEAR Protocol Hoodie</p>
    </div>
  );
}

function Container16() {
  return (
    <div className="h-[44px] relative shrink-0 w-[182.406px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[182.406px]">
        <Paragraph5 />
        <Heading7 />
      </div>
    </div>
  );
}

function Paragraph6() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$65</p>
      </div>
    </div>
  );
}

function ProductCard() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.33px] w-[419.328px]" data-name="ProductCard">
      <Container16 />
      <Paragraph6 />
    </div>
  );
}

function ImageWithFallback4() {
  return (
    <div className="absolute left-0 size-[451.328px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback4} />
    </div>
  );
}

function Icon4() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button5() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon4 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container17() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pl-0 pr-[0.016px] pt-0 size-[451.328px] top-0" data-name="Container">
      <Button5 />
    </div>
  );
}

function Icon5() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button6() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.33px] size-[36px] top-[8px]" data-name="Button">
      <Icon5 />
    </div>
  );
}

function ProductCard1() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.328px] top-0" data-name="ProductCard">
      <ImageWithFallback4 />
      <Container17 />
      <Button6 />
    </div>
  );
}

function CardContent() {
  return (
    <div className="h-[551.328px] relative shrink-0 w-[451.328px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[551.328px] relative w-[451.328px]">
        <ProductCard />
        <ProductCard1 />
      </div>
    </div>
  );
}

function Card4() {
  return (
    <div className="absolute bg-white h-[553.344px] left-0 top-0 w-[453.328px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.328px]">
        <CardContent />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Paragraph7() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Women</p>
    </div>
  );
}

function Heading8() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">Classic NEAR Tee</p>
    </div>
  );
}

function Container18() {
  return (
    <div className="h-[44px] relative shrink-0 w-[145.922px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[145.922px]">
        <Paragraph7 />
        <Heading8 />
      </div>
    </div>
  );
}

function Paragraph8() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$35</p>
      </div>
    </div>
  );
}

function ProductCard2() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.33px] w-[419.328px]" data-name="ProductCard">
      <Container18 />
      <Paragraph8 />
    </div>
  );
}

function ImageWithFallback5() {
  return (
    <div className="absolute left-0 size-[451.328px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback5} />
    </div>
  );
}

function Icon6() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button7() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon6 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container19() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pl-0 pr-[0.016px] pt-0 size-[451.328px] top-0" data-name="Container">
      <Button7 />
    </div>
  );
}

function Icon7() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button8() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.33px] size-[36px] top-[8px]" data-name="Button">
      <Icon7 />
    </div>
  );
}

function ProductCard3() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.328px] top-0" data-name="ProductCard">
      <ImageWithFallback5 />
      <Container19 />
      <Button8 />
    </div>
  );
}

function CardContent1() {
  return (
    <div className="h-[551.328px] relative shrink-0 w-[451.328px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[551.328px] relative w-[451.328px]">
        <ProductCard2 />
        <ProductCard3 />
      </div>
    </div>
  );
}

function Card5() {
  return (
    <div className="absolute bg-white h-[553.344px] left-[477.33px] top-0 w-[453.328px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.328px]">
        <CardContent1 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Paragraph9() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Accessories</p>
    </div>
  );
}

function Heading9() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">NEAR Snapback Cap</p>
    </div>
  );
}

function Container20() {
  return (
    <div className="h-[44px] relative shrink-0 w-[155.047px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[155.047px]">
        <Paragraph9 />
        <Heading9 />
      </div>
    </div>
  );
}

function Paragraph10() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$28</p>
      </div>
    </div>
  );
}

function ProductCard4() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.34px] w-[419.344px]" data-name="ProductCard">
      <Container20 />
      <Paragraph10 />
    </div>
  );
}

function ImageWithFallback6() {
  return (
    <div className="absolute left-0 size-[451.344px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback6} />
    </div>
  );
}

function Icon8() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button9() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon8 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container21() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pt-0 px-0 size-[451.344px] top-0" data-name="Container">
      <Button9 />
    </div>
  );
}

function Icon9() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button10() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.34px] size-[36px] top-[8px]" data-name="Button">
      <Icon9 />
    </div>
  );
}

function ProductCard5() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.344px] top-0" data-name="ProductCard">
      <ImageWithFallback6 />
      <Container21 />
      <Button10 />
    </div>
  );
}

function CardContent2() {
  return (
    <div className="basis-0 grow min-h-px min-w-px relative shrink-0 w-[451.344px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-full relative w-[451.344px]">
        <ProductCard4 />
        <ProductCard5 />
      </div>
    </div>
  );
}

function Card6() {
  return (
    <div className="absolute bg-white h-[553.344px] left-[954.66px] top-0 w-[453.344px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.344px]">
        <CardContent2 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Paragraph11() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Accessories</p>
    </div>
  );
}

function Heading10() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">Protocol Tote Bag</p>
    </div>
  );
}

function Container22() {
  return (
    <div className="h-[44px] relative shrink-0 w-[155.047px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[155.047px]">
        <Paragraph11 />
        <Heading10 />
      </div>
    </div>
  );
}

function Paragraph12() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$22</p>
      </div>
    </div>
  );
}

function ProductCard6() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.33px] w-[419.328px]" data-name="ProductCard">
      <Container22 />
      <Paragraph12 />
    </div>
  );
}

function ImageWithFallback7() {
  return (
    <div className="absolute left-0 size-[451.328px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback7} />
    </div>
  );
}

function Icon10() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button11() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon10 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container23() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pl-0 pr-[0.016px] pt-0 size-[451.328px] top-0" data-name="Container">
      <Button11 />
    </div>
  );
}

function Icon11() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button12() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.33px] size-[36px] top-[8px]" data-name="Button">
      <Icon11 />
    </div>
  );
}

function ProductCard7() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.328px] top-0" data-name="ProductCard">
      <ImageWithFallback7 />
      <Container23 />
      <Button12 />
    </div>
  );
}

function CardContent3() {
  return (
    <div className="h-[551.328px] relative shrink-0 w-[451.328px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[551.328px] relative w-[451.328px]">
        <ProductCard6 />
        <ProductCard7 />
      </div>
    </div>
  );
}

function Card7() {
  return (
    <div className="absolute bg-white h-[553.344px] left-0 top-[577.34px] w-[453.328px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.328px]">
        <CardContent3 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Paragraph13() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Accessories</p>
    </div>
  );
}

function Heading11() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">NEAR Coffee Mug</p>
    </div>
  );
}

function Container24() {
  return (
    <div className="h-[44px] relative shrink-0 w-[136.813px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[136.813px]">
        <Paragraph13 />
        <Heading11 />
      </div>
    </div>
  );
}

function Paragraph14() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$18</p>
      </div>
    </div>
  );
}

function ProductCard8() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.33px] w-[419.328px]" data-name="ProductCard">
      <Container24 />
      <Paragraph14 />
    </div>
  );
}

function ImageWithFallback8() {
  return (
    <div className="absolute left-0 size-[451.328px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback8} />
    </div>
  );
}

function Icon12() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button13() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon12 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container25() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pl-0 pr-[0.016px] pt-0 size-[451.328px] top-0" data-name="Container">
      <Button13 />
    </div>
  );
}

function Icon13() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button14() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.33px] size-[36px] top-[8px]" data-name="Button">
      <Icon13 />
    </div>
  );
}

function ProductCard9() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.328px] top-0" data-name="ProductCard">
      <ImageWithFallback8 />
      <Container25 />
      <Button14 />
    </div>
  );
}

function CardContent4() {
  return (
    <div className="h-[551.328px] relative shrink-0 w-[451.328px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[551.328px] relative w-[451.328px]">
        <ProductCard8 />
        <ProductCard9 />
      </div>
    </div>
  );
}

function Card8() {
  return (
    <div className="absolute bg-white h-[553.344px] left-[477.33px] top-[577.34px] w-[453.328px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.328px]">
        <CardContent4 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Paragraph15() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Exclusives</p>
    </div>
  );
}

function Heading12() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">Sticker Pack</p>
    </div>
  );
}

function Container26() {
  return (
    <div className="h-[44px] relative shrink-0 w-[109.453px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[109.453px]">
        <Paragraph15 />
        <Heading12 />
      </div>
    </div>
  );
}

function Paragraph16() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$12</p>
      </div>
    </div>
  );
}

function ProductCard10() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.34px] w-[419.344px]" data-name="ProductCard">
      <Container26 />
      <Paragraph16 />
    </div>
  );
}

function ImageWithFallback9() {
  return (
    <div className="absolute left-0 size-[451.344px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback9} />
    </div>
  );
}

function Icon14() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button15() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon14 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container27() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pt-0 px-0 size-[451.344px] top-0" data-name="Container">
      <Button15 />
    </div>
  );
}

function Icon15() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button16() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.34px] size-[36px] top-[8px]" data-name="Button">
      <Icon15 />
    </div>
  );
}

function ProductCard11() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.344px] top-0" data-name="ProductCard">
      <ImageWithFallback9 />
      <Container27 />
      <Button16 />
    </div>
  );
}

function CardContent5() {
  return (
    <div className="basis-0 grow min-h-px min-w-px relative shrink-0 w-[451.344px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-full relative w-[451.344px]">
        <ProductCard10 />
        <ProductCard11 />
      </div>
    </div>
  );
}

function Card9() {
  return (
    <div className="absolute bg-white h-[553.344px] left-[954.66px] top-[577.34px] w-[453.344px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.344px]">
        <CardContent5 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Paragraph17() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Men</p>
    </div>
  );
}

function Heading13() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">{`Men's Essential Tee - Black`}</p>
    </div>
  );
}

function Container28() {
  return (
    <div className="h-[44px] relative shrink-0 w-[246.25px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[246.25px]">
        <Paragraph17 />
        <Heading13 />
      </div>
    </div>
  );
}

function Paragraph18() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$32</p>
      </div>
    </div>
  );
}

function ProductCard12() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.33px] w-[419.328px]" data-name="ProductCard">
      <Container28 />
      <Paragraph18 />
    </div>
  );
}

function ImageWithFallback10() {
  return (
    <div className="absolute left-0 size-[451.328px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback10} />
    </div>
  );
}

function Icon16() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button17() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon16 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container29() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pl-0 pr-[0.016px] pt-0 size-[451.328px] top-0" data-name="Container">
      <Button17 />
    </div>
  );
}

function Icon17() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button18() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.33px] size-[36px] top-[8px]" data-name="Button">
      <Icon17 />
    </div>
  );
}

function ProductCard13() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.328px] top-0" data-name="ProductCard">
      <ImageWithFallback10 />
      <Container29 />
      <Button18 />
    </div>
  );
}

function CardContent6() {
  return (
    <div className="h-[551.328px] relative shrink-0 w-[451.328px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[551.328px] relative w-[451.328px]">
        <ProductCard12 />
        <ProductCard13 />
      </div>
    </div>
  );
}

function Card10() {
  return (
    <div className="absolute bg-white h-[553.344px] left-0 top-[1154.69px] w-[453.328px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.328px]">
        <CardContent6 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Paragraph19() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Men</p>
    </div>
  );
}

function Heading14() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">{`Men's Oversized Tee`}</p>
    </div>
  );
}

function Container30() {
  return (
    <div className="h-[44px] relative shrink-0 w-[173.281px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[173.281px]">
        <Paragraph19 />
        <Heading14 />
      </div>
    </div>
  );
}

function Paragraph20() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$38</p>
      </div>
    </div>
  );
}

function ProductCard14() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.33px] w-[419.328px]" data-name="ProductCard">
      <Container30 />
      <Paragraph20 />
    </div>
  );
}

function ImageWithFallback11() {
  return (
    <div className="absolute left-0 size-[451.328px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback11} />
    </div>
  );
}

function Icon18() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button19() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon18 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container31() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pl-0 pr-[0.016px] pt-0 size-[451.328px] top-0" data-name="Container">
      <Button19 />
    </div>
  );
}

function Icon19() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button20() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.33px] size-[36px] top-[8px]" data-name="Button">
      <Icon19 />
    </div>
  );
}

function ProductCard15() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.328px] top-0" data-name="ProductCard">
      <ImageWithFallback11 />
      <Container31 />
      <Button20 />
    </div>
  );
}

function CardContent7() {
  return (
    <div className="h-[551.328px] relative shrink-0 w-[451.328px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[551.328px] relative w-[451.328px]">
        <ProductCard14 />
        <ProductCard15 />
      </div>
    </div>
  );
}

function Card11() {
  return (
    <div className="absolute bg-white h-[553.344px] left-[477.33px] top-[1154.69px] w-[453.328px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.328px]">
        <CardContent7 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Paragraph21() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Men</p>
    </div>
  );
}

function Heading15() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">{`Men's Polo Shirt`}</p>
    </div>
  );
}

function Container32() {
  return (
    <div className="h-[44px] relative shrink-0 w-[145.922px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[145.922px]">
        <Paragraph21 />
        <Heading15 />
      </div>
    </div>
  );
}

function Paragraph22() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$45</p>
      </div>
    </div>
  );
}

function ProductCard16() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.34px] w-[419.344px]" data-name="ProductCard">
      <Container32 />
      <Paragraph22 />
    </div>
  );
}

function ImageWithFallback12() {
  return (
    <div className="absolute left-0 size-[451.344px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback12} />
    </div>
  );
}

function Icon20() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button21() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon20 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container33() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pt-0 px-0 size-[451.344px] top-0" data-name="Container">
      <Button21 />
    </div>
  );
}

function Icon21() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button22() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.34px] size-[36px] top-[8px]" data-name="Button">
      <Icon21 />
    </div>
  );
}

function ProductCard17() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.344px] top-0" data-name="ProductCard">
      <ImageWithFallback12 />
      <Container33 />
      <Button22 />
    </div>
  );
}

function CardContent8() {
  return (
    <div className="basis-0 grow min-h-px min-w-px relative shrink-0 w-[451.344px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-full relative w-[451.344px]">
        <ProductCard16 />
        <ProductCard17 />
      </div>
    </div>
  );
}

function Card12() {
  return (
    <div className="absolute bg-white h-[553.344px] left-[954.66px] top-[1154.69px] w-[453.344px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.344px]">
        <CardContent8 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Paragraph23() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Women</p>
    </div>
  );
}

function Heading16() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">{`Women's Fitted Tee`}</p>
    </div>
  );
}

function Container34() {
  return (
    <div className="h-[44px] relative shrink-0 w-[164.172px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[164.172px]">
        <Paragraph23 />
        <Heading16 />
      </div>
    </div>
  );
}

function Paragraph24() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$32</p>
      </div>
    </div>
  );
}

function ProductCard18() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.33px] w-[419.328px]" data-name="ProductCard">
      <Container34 />
      <Paragraph24 />
    </div>
  );
}

function ImageWithFallback13() {
  return (
    <div className="absolute left-0 size-[451.328px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback13} />
    </div>
  );
}

function Icon22() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button23() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon22 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container35() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pl-0 pr-[0.016px] pt-0 size-[451.328px] top-0" data-name="Container">
      <Button23 />
    </div>
  );
}

function Icon23() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button24() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.33px] size-[36px] top-[8px]" data-name="Button">
      <Icon23 />
    </div>
  );
}

function ProductCard19() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.328px] top-0" data-name="ProductCard">
      <ImageWithFallback13 />
      <Container35 />
      <Button24 />
    </div>
  );
}

function CardContent9() {
  return (
    <div className="h-[551.328px] relative shrink-0 w-[451.328px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[551.328px] relative w-[451.328px]">
        <ProductCard18 />
        <ProductCard19 />
      </div>
    </div>
  );
}

function Card13() {
  return (
    <div className="absolute bg-white h-[553.344px] left-0 top-[1732.03px] w-[453.328px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.328px]">
        <CardContent9 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Paragraph25() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Women</p>
    </div>
  );
}

function Heading17() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">{`Women's Crop Top`}</p>
    </div>
  );
}

function Container36() {
  return (
    <div className="h-[44px] relative shrink-0 w-[145.922px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[145.922px]">
        <Paragraph25 />
        <Heading17 />
      </div>
    </div>
  );
}

function Paragraph26() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$30</p>
      </div>
    </div>
  );
}

function ProductCard20() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.33px] w-[419.328px]" data-name="ProductCard">
      <Container36 />
      <Paragraph26 />
    </div>
  );
}

function ImageWithFallback14() {
  return (
    <div className="absolute left-0 size-[451.328px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback14} />
    </div>
  );
}

function Icon24() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button25() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon24 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container37() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pl-0 pr-[0.016px] pt-0 size-[451.328px] top-0" data-name="Container">
      <Button25 />
    </div>
  );
}

function Icon25() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button26() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.33px] size-[36px] top-[8px]" data-name="Button">
      <Icon25 />
    </div>
  );
}

function ProductCard21() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.328px] top-0" data-name="ProductCard">
      <ImageWithFallback14 />
      <Container37 />
      <Button26 />
    </div>
  );
}

function CardContent10() {
  return (
    <div className="h-[551.328px] relative shrink-0 w-[451.328px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[551.328px] relative w-[451.328px]">
        <ProductCard20 />
        <ProductCard21 />
      </div>
    </div>
  );
}

function Card14() {
  return (
    <div className="absolute bg-white h-[553.344px] left-[477.33px] top-[1732.03px] w-[453.328px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.328px]">
        <CardContent10 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Paragraph27() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Women</p>
    </div>
  );
}

function Heading18() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">{`Women's Hoodie`}</p>
    </div>
  );
}

function Container38() {
  return (
    <div className="h-[44px] relative shrink-0 w-[127.688px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[127.688px]">
        <Paragraph27 />
        <Heading18 />
      </div>
    </div>
  );
}

function Paragraph28() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$58</p>
      </div>
    </div>
  );
}

function ProductCard22() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.34px] w-[419.344px]" data-name="ProductCard">
      <Container38 />
      <Paragraph28 />
    </div>
  );
}

function ImageWithFallback15() {
  return (
    <div className="absolute left-0 size-[451.344px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback15} />
    </div>
  );
}

function Icon26() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button27() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon26 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container39() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pt-0 px-0 size-[451.344px] top-0" data-name="Container">
      <Button27 />
    </div>
  );
}

function Icon27() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button28() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.34px] size-[36px] top-[8px]" data-name="Button">
      <Icon27 />
    </div>
  );
}

function ProductCard23() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.344px] top-0" data-name="ProductCard">
      <ImageWithFallback15 />
      <Container39 />
      <Button28 />
    </div>
  );
}

function CardContent11() {
  return (
    <div className="basis-0 grow min-h-px min-w-px relative shrink-0 w-[451.344px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-full relative w-[451.344px]">
        <ProductCard22 />
        <ProductCard23 />
      </div>
    </div>
  );
}

function Card15() {
  return (
    <div className="absolute bg-white h-[553.344px] left-[954.66px] top-[1732.03px] w-[453.344px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.344px]">
        <CardContent11 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Paragraph29() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Exclusives</p>
    </div>
  );
}

function Heading19() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">Limited Edition Black Tee</p>
    </div>
  );
}

function Container40() {
  return (
    <div className="h-[44px] relative shrink-0 w-[228px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[228px]">
        <Paragraph29 />
        <Heading19 />
      </div>
    </div>
  );
}

function Paragraph30() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$48</p>
      </div>
    </div>
  );
}

function ProductCard24() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.33px] w-[419.328px]" data-name="ProductCard">
      <Container40 />
      <Paragraph30 />
    </div>
  );
}

function ImageWithFallback16() {
  return (
    <div className="absolute left-0 size-[451.328px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback16} />
    </div>
  );
}

function Icon28() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button29() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon28 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container41() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pl-0 pr-[0.016px] pt-0 size-[451.328px] top-0" data-name="Container">
      <Button29 />
    </div>
  );
}

function Icon29() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button30() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.33px] size-[36px] top-[8px]" data-name="Button">
      <Icon29 />
    </div>
  );
}

function ProductCard25() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.328px] top-0" data-name="ProductCard">
      <ImageWithFallback16 />
      <Container41 />
      <Button30 />
    </div>
  );
}

function CardContent12() {
  return (
    <div className="h-[551.328px] relative shrink-0 w-[451.328px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[551.328px] relative w-[451.328px]">
        <ProductCard24 />
        <ProductCard25 />
      </div>
    </div>
  );
}

function Card16() {
  return (
    <div className="absolute bg-white h-[553.344px] left-0 top-[2309.38px] w-[453.328px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.328px]">
        <CardContent12 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Paragraph31() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Exclusives</p>
    </div>
  );
}

function Heading20() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">{`Founder's Edition Tee`}</p>
    </div>
  );
}

function Container42() {
  return (
    <div className="h-[44px] relative shrink-0 w-[191.531px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[191.531px]">
        <Paragraph31 />
        <Heading20 />
      </div>
    </div>
  );
}

function Paragraph32() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$52</p>
      </div>
    </div>
  );
}

function ProductCard26() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.33px] w-[419.328px]" data-name="ProductCard">
      <Container42 />
      <Paragraph32 />
    </div>
  );
}

function ImageWithFallback17() {
  return (
    <div className="absolute left-0 size-[451.328px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback17} />
    </div>
  );
}

function Icon30() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button31() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon30 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container43() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pl-0 pr-[0.016px] pt-0 size-[451.328px] top-0" data-name="Container">
      <Button31 />
    </div>
  );
}

function Icon31() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button32() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.33px] size-[36px] top-[8px]" data-name="Button">
      <Icon31 />
    </div>
  );
}

function ProductCard27() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.328px] top-0" data-name="ProductCard">
      <ImageWithFallback17 />
      <Container43 />
      <Button32 />
    </div>
  );
}

function CardContent13() {
  return (
    <div className="h-[551.328px] relative shrink-0 w-[451.328px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[551.328px] relative w-[451.328px]">
        <ProductCard26 />
        <ProductCard27 />
      </div>
    </div>
  );
}

function Card17() {
  return (
    <div className="absolute bg-white h-[553.344px] left-[477.33px] top-[2309.38px] w-[453.328px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.328px]">
        <CardContent13 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Paragraph33() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Accessories</p>
    </div>
  );
}

function Heading21() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">NEAR Beanie</p>
    </div>
  );
}

function Container44() {
  return (
    <div className="h-[44px] relative shrink-0 w-[100.328px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[100.328px]">
        <Paragraph33 />
        <Heading21 />
      </div>
    </div>
  );
}

function Paragraph34() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$24</p>
      </div>
    </div>
  );
}

function ProductCard28() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.34px] w-[419.344px]" data-name="ProductCard">
      <Container44 />
      <Paragraph34 />
    </div>
  );
}

function ImageWithFallback18() {
  return (
    <div className="absolute left-0 size-[451.344px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback18} />
    </div>
  );
}

function Icon32() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button33() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon32 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container45() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pt-0 px-0 size-[451.344px] top-0" data-name="Container">
      <Button33 />
    </div>
  );
}

function Icon33() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button34() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.34px] size-[36px] top-[8px]" data-name="Button">
      <Icon33 />
    </div>
  );
}

function ProductCard29() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.344px] top-0" data-name="ProductCard">
      <ImageWithFallback18 />
      <Container45 />
      <Button34 />
    </div>
  );
}

function CardContent14() {
  return (
    <div className="basis-0 grow min-h-px min-w-px relative shrink-0 w-[451.344px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-full relative w-[451.344px]">
        <ProductCard28 />
        <ProductCard29 />
      </div>
    </div>
  );
}

function Card18() {
  return (
    <div className="absolute bg-white h-[553.344px] left-[954.66px] top-[2309.38px] w-[453.344px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.344px]">
        <CardContent14 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Paragraph35() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Accessories</p>
    </div>
  );
}

function Heading22() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">Premium Socks (3-Pack)</p>
    </div>
  );
}

function Container46() {
  return (
    <div className="h-[44px] relative shrink-0 w-[200.641px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[200.641px]">
        <Paragraph35 />
        <Heading22 />
      </div>
    </div>
  );
}

function Paragraph36() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$28</p>
      </div>
    </div>
  );
}

function ProductCard30() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.33px] w-[419.328px]" data-name="ProductCard">
      <Container46 />
      <Paragraph36 />
    </div>
  );
}

function ImageWithFallback19() {
  return (
    <div className="absolute left-0 size-[451.328px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback19} />
    </div>
  );
}

function Icon34() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button35() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon34 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container47() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pl-0 pr-[0.016px] pt-0 size-[451.328px] top-0" data-name="Container">
      <Button35 />
    </div>
  );
}

function Icon35() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button36() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.33px] size-[36px] top-[8px]" data-name="Button">
      <Icon35 />
    </div>
  );
}

function ProductCard31() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.328px] top-0" data-name="ProductCard">
      <ImageWithFallback19 />
      <Container47 />
      <Button36 />
    </div>
  );
}

function CardContent15() {
  return (
    <div className="h-[551.328px] relative shrink-0 w-[451.328px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[551.328px] relative w-[451.328px]">
        <ProductCard30 />
        <ProductCard31 />
      </div>
    </div>
  );
}

function Card19() {
  return (
    <div className="absolute bg-white h-[553.344px] left-0 top-[2886.72px] w-[453.328px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.328px]">
        <CardContent15 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Paragraph37() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Accessories</p>
    </div>
  );
}

function Heading23() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">NEAR Backpack</p>
    </div>
  );
}

function Container48() {
  return (
    <div className="h-[44px] relative shrink-0 w-[118.563px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[118.563px]">
        <Paragraph37 />
        <Heading23 />
      </div>
    </div>
  );
}

function Paragraph38() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$75</p>
      </div>
    </div>
  );
}

function ProductCard32() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.33px] w-[419.328px]" data-name="ProductCard">
      <Container48 />
      <Paragraph38 />
    </div>
  );
}

function ImageWithFallback20() {
  return (
    <div className="absolute left-0 size-[451.328px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback20} />
    </div>
  );
}

function Icon36() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button37() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon36 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container49() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pl-0 pr-[0.016px] pt-0 size-[451.328px] top-0" data-name="Container">
      <Button37 />
    </div>
  );
}

function Icon37() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button38() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.33px] size-[36px] top-[8px]" data-name="Button">
      <Icon37 />
    </div>
  );
}

function ProductCard33() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.328px] top-0" data-name="ProductCard">
      <ImageWithFallback20 />
      <Container49 />
      <Button38 />
    </div>
  );
}

function CardContent16() {
  return (
    <div className="h-[551.328px] relative shrink-0 w-[451.328px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[551.328px] relative w-[451.328px]">
        <ProductCard32 />
        <ProductCard33 />
      </div>
    </div>
  );
}

function Card20() {
  return (
    <div className="absolute bg-white h-[553.344px] left-[477.33px] top-[2886.72px] w-[453.328px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.328px]">
        <CardContent16 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Paragraph39() {
  return (
    <div className="content-stretch flex h-[16px] items-start relative shrink-0 w-full" data-name="Paragraph">
      <p className="basis-0 font-['Red_Hat_Mono:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[#717182] text-[12px] tracking-[0.6px] uppercase">Accessories</p>
    </div>
  );
}

function Heading24() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 3">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">NEAR Water Bottle</p>
    </div>
  );
}

function Container50() {
  return (
    <div className="h-[44px] relative shrink-0 w-[155.047px]" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col gap-[4px] h-[44px] items-start relative w-[155.047px]">
        <Paragraph39 />
        <Heading24 />
      </div>
    </div>
  );
}

function Paragraph40() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Paragraph">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-[28px] text-[16px] text-neutral-950 text-right top-[-1px] tracking-[-0.48px] translate-x-[-100%] w-[28px]">$26</p>
      </div>
    </div>
  );
}

function ProductCard34() {
  return (
    <div className="absolute content-stretch flex h-[44px] items-start justify-between left-[16px] top-[467.34px] w-[419.344px]" data-name="ProductCard">
      <Container50 />
      <Paragraph40 />
    </div>
  );
}

function ImageWithFallback21() {
  return (
    <div className="absolute left-0 size-[451.344px] top-0" data-name="ImageWithFallback">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImageWithFallback21} />
    </div>
  );
}

function Icon38() {
  return (
    <div className="absolute left-[12px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M3.33333 8H12.6667" id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 3.33333V12.6667" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button39() {
  return (
    <div className="bg-neutral-950 h-[36px] relative shrink-0 w-[127.281px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[36px] relative w-[127.281px]">
        <Icon38 />
        <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[20px] left-[44px] text-[14px] text-nowrap text-white top-[8px] tracking-[-0.48px] whitespace-pre">QUICK ADD</p>
      </div>
    </div>
  );
}

function Container51() {
  return (
    <div className="absolute box-border content-stretch flex items-end justify-center left-0 opacity-0 pb-[24px] pt-0 px-0 size-[451.344px] top-0" data-name="Container">
      <Button39 />
    </div>
  );
}

function Icon39() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button40() {
  return (
    <div className="absolute bg-[rgba(255,255,255,0.8)] content-stretch flex items-center justify-center left-[407.34px] size-[36px] top-[8px]" data-name="Button">
      <Icon39 />
    </div>
  );
}

function ProductCard35() {
  return (
    <div className="absolute bg-[#ececf0] left-0 overflow-clip size-[451.344px] top-0" data-name="ProductCard">
      <ImageWithFallback21 />
      <Container51 />
      <Button40 />
    </div>
  );
}

function CardContent17() {
  return (
    <div className="basis-0 grow min-h-px min-w-px relative shrink-0 w-[451.344px]" data-name="CardContent">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-full relative w-[451.344px]">
        <ProductCard34 />
        <ProductCard35 />
      </div>
    </div>
  );
}

function Card21() {
  return (
    <div className="absolute bg-white h-[553.344px] left-[954.66px] top-[2886.72px] w-[453.344px]" data-name="Card">
      <div className="box-border content-stretch flex flex-col h-[553.344px] items-start overflow-clip p-px relative rounded-[inherit] w-[453.344px]">
        <CardContent17 />
      </div>
      <div aria-hidden="true" className="absolute border border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Section1() {
  return (
    <div className="absolute h-[3440.06px] left-[75px] top-[2143px] w-[1408px]" data-name="Section">
      <Card4 />
      <Card5 />
      <Card6 />
      <Card7 />
      <Card8 />
      <Card9 />
      <Card10 />
      <Card11 />
      <Card12 />
      <Card13 />
      <Card14 />
      <Card15 />
      <Card16 />
      <Card17 />
      <Card18 />
      <Card19 />
      <Card20 />
      <Card21 />
    </div>
  );
}

function Heading3() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 4">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">NEAR Protocol</p>
    </div>
  );
}

function Paragraph41() {
  return (
    <div className="h-[40px] relative shrink-0 w-full" data-name="Paragraph">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[20px] left-0 text-[#717182] text-[14px] top-0 tracking-[-0.48px] w-[262px]">Building the future of blockchain technology, one block at a time.</p>
    </div>
  );
}

function Container52() {
  return (
    <div className="[grid-area:1_/_1] content-stretch flex flex-col gap-[16px] items-start relative shrink-0" data-name="Container">
      <Heading3 />
      <Paragraph41 />
    </div>
  );
}

function Heading25() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 4">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">Shop</p>
    </div>
  );
}

function Button41() {
  return (
    <div className="absolute h-[20px] left-0 top-0 w-[95.047px]" data-name="Button">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[20px] left-0 text-[#717182] text-[14px] text-nowrap top-0 tracking-[-0.48px] whitespace-pre">All Products</p>
    </div>
  );
}

function ListItem() {
  return (
    <div className="h-[20px] relative shrink-0 w-full" data-name="List Item">
      <Button41 />
    </div>
  );
}

function Button42() {
  return (
    <div className="absolute h-[20px] left-0 top-0 w-[87.125px]" data-name="Button">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[20px] left-0 text-[#717182] text-[14px] text-nowrap top-0 tracking-[-0.48px] whitespace-pre">Collections</p>
    </div>
  );
}

function ListItem1() {
  return (
    <div className="h-[20px] relative shrink-0 w-full" data-name="List Item">
      <Button42 />
    </div>
  );
}

function List() {
  return (
    <div className="content-stretch flex flex-col gap-[8px] h-[48px] items-start relative shrink-0 w-full" data-name="List">
      <ListItem />
      <ListItem1 />
    </div>
  );
}

function Container53() {
  return (
    <div className="[grid-area:1_/_2] content-stretch flex flex-col gap-[16px] items-start relative shrink-0" data-name="Container">
      <Heading25 />
      <List />
    </div>
  );
}

function Heading26() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 4">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">Support</p>
    </div>
  );
}

function Link() {
  return (
    <div className="absolute content-stretch flex h-[18px] items-start left-0 top-px w-[23.766px]" data-name="Link">
      <p className="font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[20px] relative shrink-0 text-[#717182] text-[14px] text-nowrap tracking-[-0.48px] whitespace-pre">FAQ</p>
    </div>
  );
}

function ListItem2() {
  return (
    <div className="h-[20px] relative shrink-0 w-full" data-name="List Item">
      <Link />
    </div>
  );
}

function Link1() {
  return (
    <div className="absolute content-stretch flex h-[18px] items-start left-0 top-px w-[63.375px]" data-name="Link">
      <p className="font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[20px] relative shrink-0 text-[#717182] text-[14px] text-nowrap tracking-[-0.48px] whitespace-pre">Shipping</p>
    </div>
  );
}

function ListItem3() {
  return (
    <div className="h-[20px] relative shrink-0 w-full" data-name="List Item">
      <Link1 />
    </div>
  );
}

function Link2() {
  return (
    <div className="absolute content-stretch flex h-[18px] items-start left-0 top-px w-[55.453px]" data-name="Link">
      <p className="font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[20px] relative shrink-0 text-[#717182] text-[14px] text-nowrap tracking-[-0.48px] whitespace-pre">Returns</p>
    </div>
  );
}

function ListItem4() {
  return (
    <div className="h-[20px] relative shrink-0 w-full" data-name="List Item">
      <Link2 />
    </div>
  );
}

function List1() {
  return (
    <div className="content-stretch flex flex-col gap-[8px] h-[76px] items-start relative shrink-0 w-full" data-name="List">
      <ListItem2 />
      <ListItem3 />
      <ListItem4 />
    </div>
  );
}

function Container54() {
  return (
    <div className="[grid-area:1_/_3] content-stretch flex flex-col gap-[16px] items-start relative shrink-0" data-name="Container">
      <Heading26 />
      <List1 />
    </div>
  );
}

function Heading27() {
  return (
    <div className="h-[24px] relative shrink-0 w-full" data-name="Heading 4">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">Connect</p>
    </div>
  );
}

function Link3() {
  return (
    <div className="absolute content-stretch flex h-[18px] items-start left-0 top-px w-[55.453px]" data-name="Link">
      <p className="font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[20px] relative shrink-0 text-[#717182] text-[14px] text-nowrap tracking-[-0.48px] whitespace-pre">Twitter</p>
    </div>
  );
}

function ListItem5() {
  return (
    <div className="h-[20px] relative shrink-0 w-full" data-name="List Item">
      <Link3 />
    </div>
  );
}

function Link4() {
  return (
    <div className="absolute content-stretch flex h-[18px] items-start left-0 top-px w-[47.531px]" data-name="Link">
      <p className="font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[20px] relative shrink-0 text-[#717182] text-[14px] text-nowrap tracking-[-0.48px] whitespace-pre">GitHub</p>
    </div>
  );
}

function ListItem6() {
  return (
    <div className="h-[20px] relative shrink-0 w-full" data-name="List Item">
      <Link4 />
    </div>
  );
}

function Link5() {
  return (
    <div className="absolute content-stretch flex h-[18px] items-start left-0 top-px w-[55.453px]" data-name="Link">
      <p className="font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[20px] relative shrink-0 text-[#717182] text-[14px] text-nowrap tracking-[-0.48px] whitespace-pre">Contact</p>
    </div>
  );
}

function ListItem7() {
  return (
    <div className="h-[20px] relative shrink-0 w-full" data-name="List Item">
      <Link5 />
    </div>
  );
}

function List2() {
  return (
    <div className="content-stretch flex flex-col gap-[8px] h-[76px] items-start relative shrink-0 w-full" data-name="List">
      <ListItem5 />
      <ListItem6 />
      <ListItem7 />
    </div>
  );
}

function Container55() {
  return (
    <div className="[grid-area:1_/_4] content-stretch flex flex-col gap-[16px] items-start relative shrink-0" data-name="Container">
      <Heading27 />
      <List2 />
    </div>
  );
}

function Container56() {
  return (
    <div className="gap-[32px] grid grid-cols-[repeat(4,_minmax(0px,_1fr))] grid-rows-[repeat(1,_minmax(0px,_1fr))] h-[116px] relative shrink-0 w-full" data-name="Container">
      <Container52 />
      <Container53 />
      <Container54 />
      <Container55 />
    </div>
  );
}

function Paragraph42() {
  return (
    <div className="h-[20px] relative shrink-0 w-full" data-name="Paragraph">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[20px] left-[704.17px] text-[#717182] text-[14px] text-center text-nowrap top-0 tracking-[-0.48px] translate-x-[-50%] whitespace-pre">© 2025 NEAR Protocol. All rights reserved.</p>
    </div>
  );
}

function Container57() {
  return (
    <div className="box-border content-stretch flex flex-col h-[53px] items-start pb-0 pt-[33px] px-0 relative shrink-0 w-full" data-name="Container">
      <div aria-hidden="true" className="absolute border-[1px_0px_0px] border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
      <Paragraph42 />
    </div>
  );
}

function Container58() {
  return (
    <div className="h-[265px] relative shrink-0 w-full" data-name="Container">
      <div className="size-full">
        <div className="box-border content-stretch flex flex-col gap-[32px] h-[265px] items-start pb-0 pt-[32px] px-[64px] relative w-full">
          <Container56 />
          <Container57 />
        </div>
      </div>
    </div>
  );
}

function Footer() {
  return (
    <div className="absolute box-border content-stretch flex flex-col h-[266px] items-start left-0 pb-0 pt-px px-[11px] top-[5631.06px] w-[1558px]" data-name="Footer">
      <div aria-hidden="true" className="absolute border-[1px_0px_0px] border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
      <Container58 />
    </div>
  );
}

function Section2() {
  return <div className="absolute h-0 left-0 top-[5897.06px] w-[1558px]" data-name="Section" />;
}

function Source() {
  return <div className="h-0 shrink-0 w-full" data-name="Source" />;
}

function Video() {
  return (
    <div className="absolute h-[600px] left-0 opacity-60 top-0 w-[1558px]" data-name="Video">
      <div className="overflow-clip rounded-[inherit] size-full">
        <div className="box-border content-stretch flex flex-col h-[600px] items-start pb-0 pl-0 pr-[1558px] pt-[1845px] relative w-[1558px]">
          <Source />
        </div>
      </div>
    </div>
  );
}

function Container59() {
  return <div className="absolute h-[600px] left-0 opacity-[0.538] top-0 w-[1558px]" data-name="Container" />;
}

function Container60() {
  return <div className="absolute bg-[rgba(0,0,0,0.4)] h-[600px] left-0 top-0 w-[1558px]" data-name="Container" />;
}

function Container61() {
  return <div className="absolute h-[600px] left-0 top-0 w-[1558px]" data-name="Container" style={{ backgroundImage: "url('data:image/svg+xml;utf8,<svg viewBox=\\\'0 0 1558 600\\\' xmlns=\\\'http://www.w3.org/2000/svg\\\' preserveAspectRatio=\\\'none\\\'><rect x=\\\'0\\\' y=\\\'0\\\' height=\\\'100%\\\' width=\\\'100%\\\' fill=\\\'url(%23grad)\\\' opacity=\\\'1\\\'/><defs><radialGradient id=\\\'grad\\\' gradientUnits=\\\'userSpaceOnUse\\\' cx=\\\'0\\\' cy=\\\'0\\\' r=\\\'10\\\' gradientTransform=\\\'matrix(0 -83.477 -83.477 0 779 300)\\\'><stop stop-color=\\\'rgba(255,255,255,0.1)\\\' offset=\\\'0.00064185\\\'/><stop stop-color=\\\'rgba(0,0,0,0)\\\' offset=\\\'0.00064185\\\'/></radialGradient></defs></svg>')" }} />;
}

function Container62() {
  return (
    <div className="absolute h-[600px] left-0 top-0 w-[1558px]" data-name="Container">
      <Video />
      <Container59 />
      <Container60 />
      <Container61 />
    </div>
  );
}

function Paragraph43() {
  return (
    <div className="absolute content-stretch flex h-[16px] items-start left-[13px] top-[5px] w-[70.203px]" data-name="Paragraph">
      <p className="font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[16px] relative shrink-0 text-[12px] text-nowrap text-white tracking-[0.6px] uppercase whitespace-pre">Exclusive</p>
    </div>
  );
}

function Container63() {
  return (
    <div className="absolute h-[26px] left-0 rounded-[3.35544e+07px] top-0 w-[96.203px]" data-name="Container">
      <div aria-hidden="true" className="absolute border border-[rgba(255,255,255,0.2)] border-solid inset-0 pointer-events-none rounded-[3.35544e+07px]" />
      <Paragraph43 />
    </div>
  );
}

function Text() {
  return (
    <div className="absolute content-stretch flex h-[92px] items-start left-[358.06px] top-[-10px] w-[132.313px]" data-name="Text">
      <p className="font-['FK_Grotesk_Variable:Bold',sans-serif] font-bold leading-[72px] relative shrink-0 text-[72px] text-nowrap text-white tracking-[-0.48px] whitespace-pre" style={{ fontVariationSettings: "'ital' 0, 'mono' 0" }}>
        ION
      </p>
    </div>
  );
}

function Heading() {
  return (
    <div className="absolute h-[144px] left-0 top-[42px] w-[768px]" data-name="Heading 1">
      <p className="absolute font-['FK_Grotesk_Variable:Bold',sans-serif] font-bold leading-[72px] left-0 text-[72px] text-nowrap text-white top-0 tracking-[-0.48px] whitespace-pre" style={{ fontVariationSettings: "'ital' 0, 'mono' 0" }}>
        NEAR LEG
      </p>
      <Text />
      <p className="absolute font-['FK_Grotesk_Variable:Bold',sans-serif] font-bold leading-[72px] left-0 text-[72px] text-nowrap text-white top-[72px] tracking-[-0.48px] whitespace-pre" style={{ fontVariationSettings: "'ital' 0, 'mono' 0" }}>
        COLLECTION
      </p>
    </div>
  );
}

function Text1() {
  return (
    <div className="absolute content-stretch flex h-[26px] items-start left-[357.13px] top-px w-[34.563px]" data-name="Text">
      <p className="font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[28px] relative shrink-0 text-[20px] text-nowrap text-white tracking-[-0.48px] whitespace-pre">New</p>
    </div>
  );
}

function Paragraph44() {
  return (
    <div className="absolute h-[28px] left-0 top-[202px] w-[576px]" data-name="Paragraph">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[28px] left-0 text-[20px] text-[rgba(255,255,255,0.9)] text-nowrap top-0 tracking-[-0.48px] whitespace-pre">Represent the NEAR Legion with</p>
      <Text1 />
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[28px] left-[391.69px] text-[20px] text-[rgba(255,255,255,0.9)] text-nowrap top-0 tracking-[-0.48px] whitespace-pre">Styles</p>
    </div>
  );
}

function Button43() {
  return (
    <div className="absolute bg-white h-[48px] left-0 top-[262px] w-[200.813px]" data-name="Button">
      <p className="absolute font-['Red_Hat_Mono:Medium',sans-serif] font-medium leading-[24px] left-[32px] text-[16px] text-black text-nowrap top-[11px] tracking-[-0.48px] whitespace-pre">Shop Collection</p>
    </div>
  );
}

function Container64() {
  return (
    <div className="absolute h-[310px] left-[75px] top-[145px] w-[768px]" data-name="Container">
      <Container63 />
      <Heading />
      <Paragraph44 />
      <Button43 />
    </div>
  );
}

function Container65() {
  return <div className="absolute bg-gradient-to-b from-[#ffffff] h-[128px] left-0 to-[rgba(0,0,0,0)] top-[472px] w-[1558px]" data-name="Container" />;
}

function VideoHero() {
  return (
    <div className="absolute bg-black h-[600px] left-0 overflow-clip top-[69px] w-[1558px]" data-name="VideoHero">
      <Container62 />
      <Container64 />
      <Container65 />
    </div>
  );
}

function App() {
  return (
    <div className="absolute h-[5897.06px] left-0 top-[-1915px] w-[1558px]" data-name="App">
      <Section />
      <Section1 />
      <Footer />
      <Section2 />
      <VideoHero />
    </div>
  );
}

function Text2() {
  return <div className="absolute left-0 opacity-0 size-0 top-[1914px]" data-name="Text" />;
}

function Text3() {
  return <div className="absolute left-0 opacity-0 size-0 top-[2882px]" data-name="Text" />;
}

function Group() {
  return (
    <div className="absolute contents inset-[27.64%_14.32%_27.65%_37.07%]" data-name="Group">
      <div className="absolute inset-[27.82%_51.59%_28.67%_37.07%]" data-name="Vector">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 14 14">
          <path d={svgPaths.p229a5640} fill="var(--fill-0, black)" id="Vector" />
        </svg>
      </div>
      <div className="absolute inset-[27.65%_37.77%_27.65%_50.13%]" data-name="Vector_2">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 14 15">
          <path d={svgPaths.p3dbe3800} fill="var(--fill-0, black)" id="Vector_2" />
        </svg>
      </div>
      <div className="absolute inset-[27.64%_25.06%_27.91%_63.74%]" data-name="Vector_3">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 13 15">
          <path d={svgPaths.p35204300} fill="var(--fill-0, black)" id="Vector_3" />
        </svg>
      </div>
      <div className="absolute inset-[28.67%_14.32%_28.67%_76.88%]" data-name="Vector_4">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11 14">
          <path d={svgPaths.p211f3380} fill="var(--fill-0, black)" id="Vector_4" />
        </svg>
      </div>
    </div>
  );
}

function NearLogo() {
  return (
    <div className="absolute contents inset-[19.05%_14.32%_19.05%_14.33%]" data-name="NEAR Logo">
      <Group />
      <div className="absolute inset-[19.05%_68.28%_19.05%_14.33%]" data-name="Vector_5">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 20 20">
          <path d={svgPaths.p7395c00} fill="var(--fill-0, black)" id="Vector_5" />
        </svg>
      </div>
    </div>
  );
}

function Icon40() {
  return (
    <div className="h-[32px] overflow-clip relative shrink-0 w-full" data-name="Icon">
      <NearLogo />
    </div>
  );
}

function NearLogo1() {
  return (
    <div className="absolute content-stretch flex flex-col h-[32px] items-start left-0 top-[2px] w-[115px]" data-name="NearLogo">
      <Icon40 />
    </div>
  );
}

function Button44() {
  return (
    <div className="h-[24px] relative shrink-0 w-[27.375px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[27.375px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">Men</p>
      </div>
    </div>
  );
}

function Button45() {
  return (
    <div className="h-[24px] relative shrink-0 w-[45.609px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[45.609px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">Women</p>
      </div>
    </div>
  );
}

function Button46() {
  return (
    <div className="basis-0 grow h-[24px] min-h-px min-w-px relative shrink-0" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-full">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">Collections</p>
      </div>
    </div>
  );
}

function Button47() {
  return (
    <div className="h-[24px] relative shrink-0 w-[45.609px]" data-name="Button">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-[24px] relative w-[45.609px]">
        <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">About</p>
      </div>
    </div>
  );
}

function Navigation() {
  return (
    <div className="absolute content-stretch flex gap-[24px] h-[24px] items-center left-[558.53px] top-[6px] w-[290.922px]" data-name="Navigation">
      <Button44 />
      <Button45 />
      <Button46 />
      <Button47 />
    </div>
  );
}

function Icon41() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p399eca00} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d={svgPaths.pc93b400} id="Vector_2" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button48() {
  return (
    <div className="content-stretch flex items-center justify-center relative shrink-0 size-[36px]" data-name="Button">
      <Icon41 />
    </div>
  );
}

function Icon42() {
  return (
    <div className="absolute left-[10px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d={svgPaths.p13f2e300} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function FavButton() {
  return (
    <div className="relative shrink-0 size-[36px]" data-name="Fav button">
      <Icon42 />
    </div>
  );
}

function Icon43() {
  return (
    <div className="absolute left-[10px] size-[16px] top-[10px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g clipPath="url(#clip0_5_1548)" id="Icon">
          <path d={svgPaths.p22b32180} id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d={svgPaths.pceec000} id="Vector_2" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d={svgPaths.pec8cb00} id="Vector_3" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
        <defs>
          <clipPath id="clip0_5_1548">
            <rect fill="white" height="16" width="16" />
          </clipPath>
        </defs>
      </svg>
    </div>
  );
}

function CartButton() {
  return (
    <div className="relative shrink-0 size-[36px]" data-name="Cart Button">
      <Icon43 />
    </div>
  );
}

function Container66() {
  return (
    <div className="absolute content-stretch flex gap-[8px] items-center left-[1284px] top-0" data-name="Container">
      <Button48 />
      <FavButton />
      <CartButton />
    </div>
  );
}

function IconSearch() {
  return (
    <div className="absolute left-[9px] size-[16px] top-[12px]" data-name="icon/search">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="icon/search">
          <path d={svgPaths.p107a080} id="Vector" stroke="var(--stroke-0, #717182)" strokeLinecap="round" strokeLinejoin="round" />
          <path d="M14 14L11.1 11.1" id="Vector_2" stroke="var(--stroke-0, #717182)" strokeLinecap="round" strokeLinejoin="round" />
        </g>
      </svg>
    </div>
  );
}

function Search() {
  return (
    <div className="absolute h-[40px] left-[1032px] top-[-2px] w-[238px]" data-name="Search">
      <div className="absolute bg-[#f3f3f5] inset-0" data-name="Searchfield" />
      <div className="absolute flex flex-col font-['Red_Hat_Mono:Regular',sans-serif] font-normal justify-center leading-[0] left-[31px] text-[#717182] text-[14px] text-nowrap top-[20px] tracking-[-0.48px] translate-y-[-50%]">
        <p className="leading-[24px] whitespace-pre">Search products...</p>
      </div>
      <IconSearch />
    </div>
  );
}

function Container67() {
  return (
    <div className="h-[36px] relative shrink-0 w-full" data-name="Container">
      <NearLogo1 />
      <Navigation />
      <Container66 />
      <Search />
    </div>
  );
}

function Header() {
  return (
    <div className="absolute bg-white box-border content-stretch flex flex-col h-[69px] items-start left-0 pb-px pt-[16px] px-[75px] top-0 w-[1558px]" data-name="Header">
      <div aria-hidden="true" className="absolute border-[0px_0px_1px] border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none" />
      <Container67 />
    </div>
  );
}

function PrimitiveDiv() {
  return <div className="absolute bg-[rgba(0,0,0,0.5)] h-[968px] left-1/2 top-[calc(50%-0.5px)] translate-x-[-50%] translate-y-[-50%] w-[1573px]" data-name="Primitive.div" />;
}

function PrimitiveH() {
  return (
    <div className="basis-0 grow min-h-px min-w-px relative shrink-0 w-[479px]" data-name="Primitive.h2">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border h-full relative w-[479px]">
        <p className="absolute font-['Red_Hat_Mono:SemiBold',sans-serif] font-semibold leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">Favorites</p>
      </div>
    </div>
  );
}

function PrimitiveP() {
  return (
    <div className="h-[20px] relative shrink-0 w-[479px]" data-name="Primitive.p">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border font-['Red_Hat_Mono:Regular',sans-serif] font-normal h-[20px] leading-[20px] relative text-[#717182] text-nowrap tracking-[-0.48px] w-[479px] whitespace-pre">
        <p className="absolute left-0 text-[14px] top-0">Your saved items</p>
        <p className="absolute left-[247.5px] text-[12px] text-center top-[445px] translate-x-[-50%]">Click the heart icon on products to save them here</p>
      </div>
    </div>
  );
}

function SheetHeader() {
  return (
    <div className="absolute box-border content-stretch flex flex-col gap-[6px] h-[82px] items-start left-px pl-[16px] pr-0 py-[16px] top-0 w-[511px]" data-name="SheetHeader">
      <PrimitiveH />
      <PrimitiveP />
    </div>
  );
}

function Icon44() {
  return (
    <div className="absolute left-[240px] size-[32px] top-[293px]" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 32 32">
        <g id="Icon">
          <path d={svgPaths.p18d31c00} id="Vector" stroke="var(--stroke-0, #717182)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" />
        </g>
      </svg>
    </div>
  );
}

function Favorites() {
  return (
    <div className="absolute h-[846px] left-px overflow-clip top-[122px] w-[511px]" data-name="Favorites">
      <p className="absolute font-['Red_Hat_Mono:SemiBold',sans-serif] font-semibold leading-[24px] left-[255.5px] text-[#717182] text-[16px] text-center text-nowrap top-[338px] tracking-[-0.48px] translate-x-[-50%] whitespace-pre">No favorites yet</p>
      <Icon44 />
    </div>
  );
}

function Icon45() {
  return (
    <div className="absolute left-0 size-[16px] top-0" data-name="Icon">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path d="M12 4L4 12" id="Vector" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M4 4L12 12" id="Vector_2" stroke="var(--stroke-0, #0A0A0A)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function SheetContent() {
  return (
    <div className="absolute left-[-1px] overflow-clip size-px top-[15px]" data-name="SheetContent">
      <p className="absolute font-['Red_Hat_Mono:Regular',sans-serif] font-normal leading-[24px] left-0 text-[16px] text-neutral-950 text-nowrap top-[-1px] tracking-[-0.48px] whitespace-pre">Close</p>
    </div>
  );
}

function PrimitiveButton() {
  return (
    <div className="absolute left-[480px] opacity-70 rounded-[2px] size-[16px] top-[16px]" data-name="Primitive.button">
      <Icon45 />
      <SheetContent />
    </div>
  );
}

function PrimitiveDiv1() {
  return (
    <div className="absolute bg-white h-[968px] left-[1061px] top-[-1px] w-[512px]" data-name="Primitive.div">
      <div aria-hidden="true" className="absolute border-[0px_0px_0px_1px] border-[rgba(0,0,0,0.1)] border-solid inset-0 pointer-events-none shadow-[0px_10px_15px_-3px_rgba(0,0,0,0.1),0px_4px_6px_-4px_rgba(0,0,0,0.1)]" />
      <SheetHeader />
      <Favorites />
      <PrimitiveButton />
    </div>
  );
}

export default function FavoritesEmpty() {
  return (
    <div className="bg-white relative size-full" data-name="Favorites (empty)">
      <App />
      <Text2 />
      <Text3 />
      <Header />
      <PrimitiveDiv />
      <PrimitiveDiv1 />
    </div>
  );
}